...
//**********************************************************************
    ...
    private void recursiveSearch(int row, int col, boolean do8WaySearch) {
        ...
        if (row < rows && row >= 0 && col < cols && col >= 0 && currWord.length() < MAX_WORD_LENGTH && !getGamePiece(row, col).hasBeenVisited()){
            Character piece = getGamePiece(row, col).getElement();
            currWord += piece;
            ...
            if(currWord.length() >= MIN_WORD_LENGTH && dictionary.contains(currWord.toLowerCase())){
                validWord(currWord);
            }
